...
app.post('/api/roster/submit', async (req, res) => {
  ...
  if (!Array.isArray(repos) || repos.length !== 3) return res.status(400).json({ success:false, error:'Exactly 3 repos required' });
  for (const r of repos) if (!/^[A-Za-z0-9_.-]+\/[A-Za-z0-9_.-]+$/.test(r)) return res.status(400).json({ success:false, error:`Invalid repo: ${r}` });
  const weekNo = getISOWeek(new Date());
